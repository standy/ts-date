...
describe('addUnits', function () {
	describe('add given amount of units', function () {
		it('addMilliseconds', function () {
			const tsDate = createTsDate(new Date(2017, Month.Jun, 29, 12, 30, 59, 100));
		...
		it('addYear', function () {
			const tsDate = createTsDate(new Date(2017, Month.Jun, 29));
	...
	describe('edge cases', function () {
		it('falls to null with invalid date', function () {
			const tsDate = createTsDate(new Date(NaN));
			const result = addYear(tsDate, 2);
